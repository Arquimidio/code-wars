...
function rot(strng) {
    return strng.split('\n').reduceRight((acc, cur) => {
      acc.push([...cur].reverse().join(''));
      return acc;
    }, []).join('\n')
}
function selfieAndRot(strng) {
    const strngArr = strng.split('\n');
    const stringLen = strngArr[0].length;
    const dots = '.'.repeat(stringLen);
    const rotted = rot(strng).split('\n').join(`\n${dots}`);
    const original = strngArr.join(`${dots}\n`);
    return original + `${dots}\n${dots}` + rotted;
}
function oper(fct, s) {
    return fct(s);
}