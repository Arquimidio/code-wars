...
function numbersOfLetters(integer, result=[]) {
  const numbers = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];
  const numStr = [...integer.toString()].map(num => numbers[num]).join('');
  if(result.length > 2 && numStr.length === result[result.length - 2].length) return [...new Set(result)];
  result.push(numStr);
  return [...numbersOfLetters(numStr.length, result)]
}