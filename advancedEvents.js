...
function Event() {
  ...
  this.unsubscribe = function(...args){
    ...
    for(let i = 0; i < argsLen; i++){
      const toDelete = args[i];
      if(handlers.includes(toDelete)){
        handlers.splice(handlers.lastIndexOf(toDelete), 1);
      }
    }
  }
  ...
  this.emit = function(...args){
    ...
    for(let i = 0; i < actualHandlers.length; i++){
      if(typeof actualHandlers[i] === 'function'){
         actualHandlers[i].call(this, ...args);
      }
    }
  } 
}