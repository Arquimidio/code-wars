...
function infected(s) {
  ...
  let infectedBlocksLength = 0;
  const infectedS =  s.split('X')
  infectedS.forEach(block => block.includes('1')? infectedBlocksLength += block.length: null)
  return infectedBlocksLength / infectedS.join('').length * 100;
}