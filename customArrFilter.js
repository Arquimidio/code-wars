...
const isValid = elt => typeof elt === 'number' && Number.isInteger(elt);

Array.prototype.even = function(){
  return this.filter(num => isValid(num) && num % 2 === 0);
}
...
Array.prototype.under = function(x){
  return this.filter(num => isValid(num) && num < x);
}
...
Array.prototype.inRange = function(min,max){
  return this.filter(num => isValid(num) && num >= min && num <= max);
}