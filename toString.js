...
Array.prototype.toString = function(){
  const elements = this.map(elt => {
    if(typeof elt === 'string') 
      return `'${elt}'`;
    else
      /*
       We are changing only Array toString function, so for other data types, it works as usual
       Here, if element is boolean or number, it will invoke the usual toString
       Now if element is Array, it will invoke the new Array.toString() recursively
      */
      return elt.toString();
  })
