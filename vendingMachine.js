...
VendingMachine.prototype.vend = function (selection, itemMoney){
  ...
  //Possible problems handling
  const incorrectItem = !item;
  const notEnoughMoney = itemMoney < price;
  const outOfStock = quantity < 1;
  if(incorrectItem) return `Invalid selection! : Money in vending machine = ${this.money.toFixed(2)}`
  if(notEnoughMoney) return "Not enough money!";
  if(outOfStock) return `${name}: Out of stock!`;
  
  // If everything goes well, performs the vending / change operations
  item.quantity--;
  this.money += price;
  const change = itemMoney - price;
  return `Vending ${name}` + (change? ` with ${change.toFixed(2)} change.` : '')
};