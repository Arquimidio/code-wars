...
function spyOn (func) {
  ...
  function returnedSpied(...args){
    callCounting++;
    calledArgs.push(...args);
    const returnedVal = func(...args);
    returnedValues.push(returnedVal);
    return returnedVal;
  }
  
  returnedSpied.callCount = _ => callCounting;
  returnedSpied.wasCalledWith = argument => calledArgs.includes(argument);
  returnedSpied.returned = argument => returnedValues.includes(argument);
  return returnedSpied;
}