...
function maxSum(arr,range){
  const sumNum = (acc, cur) => acc + cur;
  const rangeValues = range.map((singleRange) => arr.slice(singleRange[0], singleRange[1] + 1).reduce(sumNum, 0));
  return Math.max(...rangeValues);
}